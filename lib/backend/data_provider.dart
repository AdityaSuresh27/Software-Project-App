...
AttendanceStats getAttendanceStats(String courseName) {
  ...
  for (var record in records) {
    switch (record.status) {
      case AttendanceStatus.present:
        present += record.periodCount;
        break;
      case AttendanceStatus.absent:
        absent += record.periodCount;
        break;
      case AttendanceStatus.late:
        late += record.periodCount;
        break;
  ...
  final totalClasses = records.fold<int>(0, (sum, r) => sum + (r.status == AttendanceStatus.cancelled ? 0 : r.periodCount));
